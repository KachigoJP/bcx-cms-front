...
// Sourece
import { LanguageType } from "helpers/interfaces";
...
const Languages = () => {
  ...
  const [updateItem, setUpdateItem] = React.useState<LanguageType>();
  ...
  const onClickUpdate = (setting: LanguageType) => () => {
  ...
  const clickConfirmDelete = () => {
    sendDeleteRequest({
      method: "delete",
      url: `${API.LANGUAGES}/${deleteId}`,
    });
    setShowDeleteModal(false);
    setShowResultModal(true);
  };

  const tableData = stateGet?.data?.data;
  ...
  return (
    <div className="main-content h-100">
      {stateGet.isError && stateGet.errors ? (
        <Alert variant="danger">
          {stateGet.errors.map((field: FieldError, key: number) => {
            return (
          ...
          >
            ...
            <tbody>
              {tableData && tableData.totalItem > 0 ? (
                tableData.data.map((item: LanguageType, key: number) => {
                  return (
                    <tr key={key}>
                      <td>{startIdx + (key + 1)}</td>
                      <td>{item.name}</td>
                      <td>{item.code}</td>
                      <td>{item.is_default?.toString()}</td>
                      <td>{item.direction?.toUpperCase()}</td>
