...
const ListUser: React.FC = () => {
  ...
  const [accountLevel, setAccountLevel] = React.useState<string | null>(
    searchParams.get('accountLevel')
  )
  const [applicantType, setApplicantType] = React.useState<string | null>(
    searchParams.get('applicantType')
  )
  ...
  const onBlurSearch = (event: any) => {
    ...
    if (!accountLevel) delete params.accountLevel
    if (!applicantType) delete params.applicantType
  ...
  const onChangeAccountLevel = (event: any) => {
    setAccountLevel(event.target.value || null)
    setPage(1)
    const params: any = {
      page: 1,
      search,
    ...
    if (!event.target.value) delete params.accountLevel
  ...
  const onChangeApplicantType = (event: any) => {
    setApplicantType(event.target.value || null)
    setPage(1)
    const params: any = {
      page: 1,
      search,
    ...
    if (!event.target.value) delete params.applicantType
        ...
        <Card.Body>
          <Card.Title>
            <Row>
              ...
              <Col>
                <Form.Group as={Row} className="me-5">
                  <Form.Label column>{t('Account Level')}</Form.Label>
                  <Form.Select
                    onChange={onChangeAccountLevel}
                    defaultValue={accountLevel || undefined}
                  >
              ...
              <Col>
                <Form.Group as={Row} className="me-5">
                  <Form.Label column>{t('Applicant Type')}</Form.Label>
                  <Form.Select
                    onChange={onChangeApplicantType}
                    defaultValue={applicantType || undefined}
                  >
                    <option value="">{t('All')}</option>
                  ...
                  tableData.data.map((item: any, key: number) => {
                    return (
                      <tr key={key}>
                        ...
                        <td>
                          <Link
                            to={generatePath(ROUTES.ADMIN_UPDATE_USER, {
                              id: item.id,
                            })}
                          >
